                                ...
                                <tr>
                                    <td class="text-center">
                                        <form action="{{route('wishlist.destroy',$wishlist->rowId)}}" method="POST">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="text-gray-32 font-size-26 remove_cart">×
                                            </button>
                                        </form>
                                    </td>
                                    ...
                                    <td class="d-none d-md-table-cell">
                                        <a href="#"><img class="img-fluid max-width-100 p-1 border border-color-1"
                                                         src="{{$wishlist->model->getFirstMediaUrl('products')}}"
                        ...
                        @else
                            @forelse($wishlists as $wishlist)
                                <tr>
                                    ...
                                    <td class="d-none d-md-table-cell">
                                        <a href="#"><img class="img-fluid max-width-100 p-1 border border-color-1"
                                                         src="{{$wishlist->product->getFirstMediaUrl('products')}}"
                                                         alt="Image Description"></a>
                                    </td>

                                    <td data-title="Product">
                                        <a href="#" class="text-gray-90">{{$wishlist->product->title}}</a>
                                    </td>

                                    <td data-title="Unit Price">
                                        <span class="">${{$wishlist->product->price}}</span>
                                    </td>
                                    ...
                                    <td>
                                        <form action="{{route('switch_to_cart',$wishlist->id)}}" method="POST">
                                            @csrf
                                            <button type="submit"
                                                    class="btn btn-soft-secondary mb-3 mb-md-0 font-weight-normal px-5 px-md-4 px-lg-5 w-100 w-md-auto">
                                                Add to Cart
